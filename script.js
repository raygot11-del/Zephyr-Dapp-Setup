...
const RECEIVER_PUBLIC_KEY = "11111111111111111111111111111112"; // Example: REPLACE with real public key
const REQUIRED_HOLD = "100000"; // used for display, not enforced client-side
const RPC_ENDPOINT = "https://api.mainnet-beta.solana.com"; // Or devnet: https://api.devnet.solana.com
...
async function run401() {
  try {
    ...
    // Encode message into a memo instruction (Solana way to "sign" arbitrary data)
    ...
    const transaction = new solanaWeb3.Transaction().add(memoIx);
...
async function run402() {
  try {
    ...
    const userTokenAccount = await splToken.getAssociatedTokenAddress(
      new solanaWeb3.PublicKey(VELOCITY_TOKEN_MINT),
      publicKey
    );
    ...
    const receiverTokenAccount = await splToken.getAssociatedTokenAddress(
      new solanaWeb3.PublicKey(VELOCITY_TOKEN_MINT),
      new solanaWeb3.PublicKey(RECEIVER_PUBLIC_KEY)
    );
